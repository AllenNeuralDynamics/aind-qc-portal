    ...
    for asset_name in asset_names:
        try:
            ...
            if vd.dataframe is not None and not vd.dataframe.empty:
                ...
                results[asset_name] = {"status": "error", "error": "No QC metrics found in dataframe", "asset_name": asset_name}
        except Exception as e:
            results[asset_name] = {"status": "error", "error": str(e), "traceback": traceback.format_exc(), "asset_name": asset_name}
    return results
    ...
    for asset_name in asset_names:
        try:
            ...
            if not bucket or not prefix:
                results[asset_name] = {"status": "error", "error": "Missing S3 bucket or prefix", "asset_name": asset_name}
                continue
            ...
            if not files:
                results[asset_name] = {"status": "error", "error": "No media files found in S3", "asset_name": asset_name}
                continue
            file_key = files[0]
            try:
